function  [o1,o2,o3] = mcI_interval( i1, i2, i3, ui1, ui2, ui3 )
% add an interval as uncertainty to the robot data
% only defined for mcI_to_rbi now 
% ui1, ui2, ui3 should be percentage of uncertainty 
...
if nargin == 1
  ...
  o1 = mcI_to_rbi( i1, i2, i3, ui1, ui2, ui3 );
end


function  rbi = mcI_to_rbi( m, c, I, um, uc, uI )

if length(c) == 3			% spatial
  m = m * infsup(1 - um, 1 + um);
  c = c * infsup(1 - uc, 1 + uc);
  I = I * infsup(1 - uI, 1 + uI);

  C = skew(c);

  rbi = infsup(zeros(6,6), zeros(6,6));
  rbi(1:3,1:3) = I + m*C*C';
  rbi(1:3,4:6) = m*C;
  rbi(4:6,1:3) = m*C';
  rbi(4:6,4:6) = m*eye(3);
else					% planar, not defined for interval
